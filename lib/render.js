...
<style>
  ...
  .card { background: var(--rc-warm-white, #FAF9F6); border: 1px solid var(--rc-border, rgba(17,17,17,.1)); border-radius: 10px; padding: 28px; transition: transform .25s cubic-bezier(.16,1,.3,1), box-shadow .25s ease; }
  .card:hover { box-shadow: 0 4px 16px rgba(0,0,0,.06); transform: translateY(-2px); }
  .section-label { font-size: 11px; letter-spacing: .2em; text-transform: uppercase; font-weight: 700; color: #183D35; }
  .check-item { display: flex; align-items: flex-start; gap: 10px; padding: 6px 0; }
  .check-icon { color: #183D35; font-size: 18px; flex-shrink: 0; margin-top: 1px; font-family: 'Material Symbols Outlined'; }
  .rc-site-nav-link.is-active { color: #183D35; font-weight: 700; }
  .price { font-size: 34px; font-weight: 800; color: #111111; letter-spacing: -.03em; }
  .price-unit { font-size: 14px; font-weight: 500; color: #5F5D58; }
...
function airbnbPricing() {
  const setup = C.AIRBNB_SETUP[0];
  return `<div class="card">
  <h3 class="font-bold text-lg text-primary mb-2">${esc(setup.name)}</h3>
  <p class="price">from ${inr(setup.price)}+</p>
  <p class="price-unit mb-4 block">${esc(setup.unit)} · depending on property size and scope</p>
  ${setup.includes.map(i => `<div class="check-item"><span class="check-icon">check</span><span class="text-sm">${esc(i)}</span></div>`).join('\n  ')}
  <p class="text-sm text-on-surface-variant mt-6 leading-relaxed">Higher-scope setup can also include: ${C.AIRBNB_SETUP_EXTENDED.map(i => esc(i).toLowerCase()).join(', ')}. These additional services are quoted according to scope.</p>
</div>

<div class="card mt-8">
  <h3 class="font-bold text-lg text-primary mb-2">Full Airbnb / Accommodation Management</h3>
  <p class="price">${C.AIRBNB_MANAGEMENT.rate}<span class="price-unit"> ${esc(C.AIRBNB_MANAGEMENT.basis)}</span></p>
  <p class="text-sm text-on-surface-variant mt-2 mb-5">Minimum ${inr(C.AIRBNB_MANAGEMENT.minimum)}/month per property. Large properties, villas, hotels and portfolios: Talk to Sales.</p>
